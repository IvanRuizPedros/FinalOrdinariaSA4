...
class ResourceBooking(models.Model):
    ...
    prioritari = fields.Boolean(string="Prioritari", default=False)
    localitzacio = fields.Selection(
        [
            ('sala', 'Sala'),
            ('punt_de_venda', 'Punt de venda')
        ], 
        string="Localització", 
        default='sala', 
        help=(
            "Defines the location of the booking."
        ),
    )
    ...
    def _get_kanban_view(self):
        # Este es un ejemplo si necesitas lógica adicional para la vista Kanban
        kanban_data = self.env["resource.booking"].search([("state", "=", "scheduled")])
        return {
            "type": "ir.actions.act_window",
            "name": "Resource Booking Kanban",
            "res_model": "resource.booking",
            "view_mode": "kanban",
            "view_id": self.env.ref("resource_booking_kanban_view").id,
            "target": "current",
        }
